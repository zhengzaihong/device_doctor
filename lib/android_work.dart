...
class AndroidWork {
  ...
  // ///获取Android运行的进程
  // Future<dynamic> getRunningAppProcesses() async =>  _channel.invokeMethod('getRunningAppProcesses');
  //
  // ///检查当前设备是否是模拟器 返回值大于3 基本就可以断定为 模拟器了
  // Future<dynamic> isEmulator() async =>  _channel.invokeMethod('checkDeviceIsEmulator');
  ...
  /// 获取设备唯一标识
   Future<dynamic> get deviceIMEINumber async {
    return  _channel.invokeMethod("getIMEINumber");
  }

  /// 获取设备API级
   Future<dynamic> get apiLevel async {
    return  _channel.invokeMethod("getAPILevel");
  }
  ...
  /// 获取设备厂商
   Future<dynamic> get deviceManufacturer async {
    return  _channel.invokeMethod("getManufacturer");
  }
  ...
  /// 获取产品名称
   Future<dynamic> get productName async {
    return _channel.invokeMethod("getProduct");
  }
